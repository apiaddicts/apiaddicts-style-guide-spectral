...
test('apiq:OAR037 should find errors', () => {
  return linter.run(oar037fail).then((results) => {
    ...
    expect(results[2].message).toBe('OAR037: Schema with type string has unrecognized format: binary');
    expect(results[3].path.join(".")).toBe('paths./invoices.post.requestBody.content.application/json.schema.properties.byte.format');
    expect(results[3].message).toBe('OAR037: Schema with type string has unrecognized format: byte');
    expect(results[4].path.join(".")).toBe('paths./invoices.post.responses.200.content.application/json.schema.properties.byte.format');
    expect(results[4].message).toBe('OAR037: Schema with type string has unrecognized format: byte');
    ...
    expect(results[5].message).toBe('OAR037: Schema with type string has unrecognized format: binary');
    ...
    expect(results[8].message).toBe('OAR037: Schema with type string has unrecognized format: binary');
  });
});
