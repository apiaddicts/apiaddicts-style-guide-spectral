...
const oar035optout = require('./OAR035/optout-example');
...
test('apiq:OAR035 should honor an operation-level `security: []` opt-out (Swagger 2.0)', () => {
  return linter.run(oar035optout).then((results) => {
    expect(results.length).toBe(1);
    expect(results[0].path.join('.')).toBe('paths./secured.get.responses');
  });
});
