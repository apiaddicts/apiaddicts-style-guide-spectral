...
test('apiq:OAR079 should report a path-item-level shared parameter with no operation-level parameters at all (Swagger 2.0)', () => {
...
test('apiq:OAR079 should not report a shared path-item parameter once the operation has a 404 (Swagger 2.0)', () => {
  return linter.run(oar079okSharedPathItemParameter).then((results) => {
    expect(results.length).toBe(0);
  });
});

test('apiq:OAR079 should evaluate shared and operation-level path parameters correctly across a large multi-path document (Swagger 2.0)', () => {
