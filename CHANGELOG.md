...
## [1.5.0-beta.5] - 2026-09-10

### Fixed

- OAR041 - UndefinedAuthTypeForWso2Scope - Rewritten entirely to match Sonar's `OAR041UndefinedAuthTypeForWso2ScopeCheck`. Previously: validated the misspelled field `x-aut-type` (missing the "h") instead of the real `x-auth-type`; only looked at the `get` verb via `given: "$.paths[*].get"` instead of every operation; and used a bare `field`/`truthy` check with no condition on `x-scope` being present at all, so it fired on any `get` missing `x-aut-type` regardless of whether the operation even declared a scope - completely unrelated to what the rule's own description promised (and that description itself talked about the `x-wso2-security` catalog structure, content copied from OAR002/OAR005, not about `x-auth-type`). Replaced with a new `apq-wso2-auth-type-required` function, run with `resolved: false` and `given: "$"` like `apq-wso2-scope-defined` (OAR005), walking every operation across `paths`, `webhooks`, `components.pathItems` and `components.callbacks` for the document's own OpenAPI version - `trace` from 3.0, `webhooks`/`pathItems` from 3.1, `query`/`additionalOperations` from 3.2 - and flagging one only when the operation's own `x-scope` key is present (any value, including blank or explicit `null` - matching Sonar's `isMissing()` presence check, not a truthiness check) and `x-auth-type` is not. Message aligned verbatim with Sonar: `OAR041: WSO2 x-scope requires x-auth-type definition`. Added `documentationUrl` and rewrote `docs/resources/OAR041.md` to match the corrected description and examples.
- OAR043 - ParsingError - Fixed `apq-validate-structure`'s `type` check, which compared `String(given)` against the valid-types list - a `type` written as an OpenAPI 3.1/3.2 array (e.g. `type: [string, "null"]`) serializes to `"string,null"`, which never matches, so every nullable array-form type was a false positive. Cross-checking against Sonar's own grammar validator (via the `IssueDumper` harness, see the Sonar changelog) turned up two further, more fundamental divergences, both now matched exactly: (1) Sonar's OpenAPI 3.1/3.2 grammar does not validate the `type` keyword at all - not a scalar, not an array's entries, in any parameter surface (`paths`, `webhooks`, `components.pathItems`, `components.parameters`) - so this rule now skips the `type` check entirely for 3.1/3.2 documents instead of enforcing an enum Sonar never does; (2) OpenAPI 3.2 accepts a new `in` value, `querystring`, rejected by Sonar's 3.0/3.1 grammar but accepted for 3.2, now reflected in a version-specific valid-values list. Also: extended `given` to cover `webhooks` and `components.pathItems` for `in` (validated consistently by Sonar across every surface and version, confirmed empirically) and added a Swagger 2.0-only `parameters.*.type` path (non-body OAS2 parameters carry `type` directly, not under `schema`, previously uncovered); a parameter reached only through a `$ref` (e.g. from `paths` to `components.parameters`) is now reported once, at its definition, instead of once at each JSONPath that happens to resolve to it (matching Sonar, which parses the document once); and a Swagger 2.0 invalid `in` value is now anchored at the parameter object's own line rather than the `in:` field's line, matching a Swagger-2-specific quirk of Sonar's grammar (an unresolvable body-vs-other-param alternation is reported at the parameter node, not the field, while OpenAPI 3.x's plain `in` enum check - and Swagger 2.0's own `type` check - are both reported at their own field line, unchanged). Added `documentationUrl` and created `docs/resources/OAR043.md` (previously missing), documenting the rule's real, narrower scope and the scope note that this rule cannot detect a document that fails to parse at all - Spectral never runs a ruleset over a document its own parser can't load - or duplicate YAML keys, both of which Sonar's grammar-level `OAR043` does report.
- OAR026 - TotalParameterDefaultValue - The `then.field: "schema.default"` was hardcoded, so on a Swagger 2.0 document (where the default lives directly on the parameter, not under `schema`) the rule silently never fired regardless of the actual default value - confirmed by the total absence of an `oap2` test suite for this rule. Rewritten as a new `apq-total-param-default-value` function that reads `default` (OAS2) or `schema.default` (OAS3/3.1/3.2) depending on `context.document.data.swagger`, matching Sonar's version-aware field lookup exactly; `given` (GET operations, `in: query`, name `$total`) is unchanged.
- OAR031 - Examples - `collectPropertyIssues` compared `schema.type`/property `type` with strict `===` against the strings `'object'`/`'array'`, which never matches OpenAPI 3.1/3.2 nullable array-form types (`type: ['object', 'null']` / `type: ['array', 'null']`) - such a property was wrongly treated as an unrecursable leaf (flagged itself, its own nested properties never checked). Added a `typeIncludes` helper (`Array.isArray(type) ? type.includes(name) : type === name`) used in both the schema-level and per-property type checks, matching Sonar's `JsonNodeUtils.isType`, which already iterates array-form `type`. Also, found while cross-checking against Sonar: Swagger 2.0 non-`body` parameters (query/path/header/formData) are now exempt from the parameter-level check - that field doesn't exist in the OAS2 parameter spec (only `default` does), so flagging it on every such parameter in any real Swagger 2.0 document was pure noise; Sonar already had this exemption.
- OAR005 - UndefinedWso2ScopeUse - The valid scope list was hardcoded to a fixed `read`/`write` enum instead of being read from the document. Rewritten as a new `apq-wso2-scope-defined` function that resolves valid scope names from `x-wso2-security.apim.x-wso2-scopes[].name` (never `key`, exact and case-sensitive), following document-local `$ref`s on the security node and on each scope element (chained, cycle-guarded) and accepting both the array and the mapping form of the container, matching Sonar's `AbstractWso2ScopesCheck` (used by OAR002/OAR003/OAR004/OAR040). The rule now runs with `resolved: false` and `given: "$"`, walking the raw document itself to reach every location Sonar's `OPERATION` grammar node reaches across Swagger 2.0, OpenAPI 3.0, 3.1 and 3.2 - `trace` from 3.0, operations nested in `callbacks` at any depth and in `components.callbacks`, `webhooks`, `components.webhooks` and `components.pathItems` from 3.1, and the `query` verb plus `additionalOperations` from 3.2 - while never firing on non-operation path-item children or on constructs the document's own declared version does not have. An `x-scope` written without a value no longer raises (matching Sonar's Missing-node behavior); an explicit `null`, in any of its YAML spellings (`~`/`Null`/`NULL` included), still does. Message aligned verbatim with Sonar: `OAR005: WSO2 scope definition does not exists`. Added `documentationUrl`.
- OAR007 - UndefinedResponseMediaType - The rule was a plain `given`/`truthy` check on `content`, evaluated unconditionally on every OpenAPI version. Since Swagger 2.0 responses never have a `content` key (they use `produces`), this made the rule flag every non-`204` response on any Swagger 2.0 document, regardless of whether `produces` was correctly declared - a total divergence from Sonar on OAS2. Rewritten as a new `apq-undefined-response-media-type` function, driven by a `given` array (one JSONPath per node shape, instead of a single higher-level match with a manually-built sub-path) so it branches like Sonar's `OAR007UndefinedResponseMediaTypeCheck`: for Swagger 2.0, checks `produces` (operation-level or global fallback) on `post`/`put`/`patch` operations only (`get`/`delete`/etc. are exempt, matching Sonar); for OpenAPI 3.x, checks each response's `content` map is non-empty (an empty `content: {}` object is now correctly flagged - the old `truthy` check treated it as compliant since `!{}` is `false` in JS), and now also covers `webhooks` (OpenAPI 3.1+), which Sonar already treated the same as `paths`. Matching each response node directly via its own `given` entry (rather than constructing `path: [...context.path, 'responses', code]` from an operation-level match) is also what lets a `$ref`'d response missing `content` report on the referenced definition's line, matching Sonar, instead of on the `$ref:` usage site. Also fixed `docs/resources/OAR007.md`: paragraph 1 wrongly stated "the request data formats must be defined" and referenced the `"consumes"` keyword (copy-pasted from OAR006's doc); corrected to describe response media types and the `"produces"` keyword, matching Sonar's corrected `OAR007.html` and the already-correct Spanish translation.
- OAR010 - DefaultResponseMediaType - `apq-response-media-type` no longer hardcodes `application/json`; added configurable `default-media-type`/`media-type-exceptions` options. Already compared media types case-insensitively; Sonar's `AbstractDefaultMediaTypeCheck` has been aligned to match (see Sonar changelog). Also fixed several real divergences from Sonar's `AbstractDefaultMediaTypeCheck`: (1) the Swagger 2.0 branch checked every operation's `produces`/global fallback with no verb filter - `get`/`delete`/etc. are now correctly exempt, matching Sonar's `post`/`put`/`patch`-only check; (2) `given` is now an array (one JSONPath per node shape - operation, response, `content` map, each matched directly, plus `webhooks` counterparts) instead of a single operation-level match with a manually-built sub-path, which is what let a "value present but wrong" response report on the parent response node instead of the `content:`/`produces:` key, and let a `$ref`'d response report on the `$ref:` usage site instead of the referenced definition; both now match Sonar exactly.

## [1.5.0-beta.4] - 2026-09-10
