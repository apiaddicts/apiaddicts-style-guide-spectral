...
Added a cross-engine parity test suite (`pruebas/*.test.js`, harness in `pruebas/helpers/cross-engine.js`) that runs the same fixture through Spectral and through the equivalent Sonar check (via a small runner, `apiaddicts.sonar.openapi.tools.IssueDumper`, in the sibling sonaropenapi-rules repo) and asserts they report the exact same issue count on the exact same lines — for every rule fixed in this release, with both default and custom configuration. Running it surfaced three more bugs, invisible to either engine's own test suite in isolation since each only compared against itself:

- OAR038 - StandardCreateResponse - `apq-valid-response-schema` returned its error `path` without prefixing it with the actual location of the response schema being checked, so Spectral always reported every OAR038 issue on line 1 instead of the real line.
- OAR082 - BinaryOrByteFormat - `apq-binary-format-check` only flagged a checked field when its `format` was present but wrong; a field missing `format` entirely (which does not satisfy "must define a byte or binary format" either) passed silently. Also aligned the reported line with Sonar's (the property's `type` line, not its name).
- OAR040 - StandardWso2ScopesName - the hardcoded default pattern (`^apfc_sc_.*`) was narrower than Sonar's own default (`^[a-zA-Z]{4,}_(SC|sc)_[a-zA-Z0-9]{1,}$`, case-insensitive on `SC`/`sc` and not tied to the literal "apfc" prefix) — confirmed by Sonar's own committed test fixtures, which already treat non-"apfc" prefixed names as valid. Spectral's default now matches Sonar's exactly.

Extended the parity suite to OpenAPI 2.0 (Swagger) fixtures for all 9 rules above. That surfaced two rules that never evaluated Swagger 2.0 documents at all in Spectral, unlike Sonar (which already had v2 test coverage for both):

- OAR038 - StandardCreateResponse - `given` only matched `content.<mediaType>.schema`, an OpenAPI 3-only construct; Swagger 2.0 responses declare `schema` directly, so OAR038 silently never ran against Swagger 2.0 specs. `given` is now a two-path array covering both shapes.
- OAR085 - OpenAPIVersion - `given` only matched `$.openapi`; Swagger 2.0 documents declare `$.swagger` instead, so OAR085 never validated a Swagger 2.0 document's version at all. `given` is now `["$.openapi", "$.swagger"]`.

Also replaced several `pruebas/edge-cases/*-oap2-*.yaml` fixtures (OAR014, OAR020, OAR021, OAR038) that were actually OpenAPI 3.0 documents wearing an "OAP2" label — checked in with a comment explaining they were an OAP3 "wrapper" worked around some earlier Spectral/Swagger-YAML limitation. That limitation could not be reproduced against genuine `swagger: "2.0"` documents with the current code, so those fixtures were rewritten as real Swagger 2.0 specs (non-body parameters using `type` directly instead of `schema.type`, `$ref`s under `#/parameters/...` instead of `#/components/parameters/...`).

### Changed

- OAR019/OAR020/OAR021 - $select/$expand/$exclude - `paths` now takes literal path segments (e.g. `/status`), not regular expressions, in both Sonar and Spectral — each segment is matched as itself, bounded by `/` or the end of the path, so `/status` matches `/status` and `/api/status` but not `/status-report` or `/user-status`. Previously (Sonar `AbstractQueryParameterCheck.parsePaths` and the Spectral function alike) each entry was compiled as a raw regex the caller had to escape correctly. As part of this, the default exclusion list's `status`/`health`/`ping` entries — an unvalidated fix from an earlier commit that matched those words as an unanchored substring anywhere in the path — are now segment-anchored like `/me` already was (default is now `/me,/health,/ping,/status` in Sonar and `/me;/health;/ping;/status` in Spectral). This is a real behavior change: a path like `/subscription-status-reports` is no longer treated as a health-check endpoint and excluded from the rule — it's evaluated like any other collection endpoint.
